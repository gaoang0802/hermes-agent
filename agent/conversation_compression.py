    ...
    *,
    ...
    if getattr(agent.context_compressor, "_last_compress_aborted", False):
        ...
        if getattr(agent, "_last_compression_summary_warning", None) != _err:
            ...
            agent._emit_warning(
                t("run_agent.compression_aborted", error=str(_err))
            )
    ...
    if summary_error:
        if getattr(agent, "_last_compression_summary_warning", None) != summary_error:
            ...
            agent._emit_warning(
                t("run_agent.compression_summary_failed", error=str(summary_error))
            )
    else:
        ...
        if _aux_fail_model:
            ...
            if getattr(agent, "_last_aux_fallback_warning_key", None) != _aux_key:
                ...
                agent._emit_warning(
                    t("run_agent.compression_aux_failed",
                      model=str(_aux_fail_model),
                      error=str(_aux_fail_err or 'unknown error'))
                )
